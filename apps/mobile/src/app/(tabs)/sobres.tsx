...
import { AppIcon, Card, IconButton, Pill, ProgressBar, ScalePressable, Screen, uiStyles, useAppTheme } from '@/components/ui';
import { money, type Envelope } from '@/data/demo';
import { useActiveLedger } from '@/store/ledger';

export default function EnvelopesScreen() {
  const theme = useAppTheme();
  const { envelopes, ledger } = useActiveLedger();
  const incomeEnvelopes = envelopes.filter((item) => item.kind === 'income');
  ...
  const isEmptyBook = expenseBudget === 0 && incomeExpected === 0 && expenseSpent === 0 && incomeReceived === 0;

  return (
    <Screen withTabBar title="Sobres" subtitle={`Presupuesto · ${ledger.name}`} right={<IconButton icon="plus" label="Crear sobre" />}>
      <Card style={[styles.hero, { backgroundColor: theme.primary }]}>
        <View style={uiStyles.between}>
          <View style={styles.heroCopy}>
            <Text style={styles.heroLabel}>Disponible para gastar</Text>
            <Text style={styles.heroValue}>{money(available)}</Text>
          </View>
        ...
        <View style={uiStyles.between}>
          ...
          <Text style={styles.heroSmall}>
            {expenseBudget > 0 ? `${money(expenseBudget)} asignados` : 'Sin presupuesto asignado'}
          </Text>
        </View>
      </Card>
      ...
      {isEmptyBook ? (
        <Card>
          <Text style={[styles.tipTitle, { color: theme.text }]}>Libro nuevo</Text>
          <Text style={[styles.small, { color: theme.muted }]}>
            Asigna montos a tus sobres o crea uno con + para empezar el presupuesto de {ledger.name}.
          </Text>
        </Card>
      ) : (
    ...
    <View style={styles.sectionBlock}>
      ...
      <View style={styles.list}>
        {items.map((envelope, index) => {
          ...
          const ratio = envelope.budget > 0 ? envelope.spent / envelope.budget : 0;
          const percent = Math.round(ratio * 100);
          const warning = mode === 'expense' && ratio > 0.85;
          const barColor = warning ? theme.warning : envelope.color;
          const amountLabel = mode === 'income' ? 'recibido' : 'disponible';
          const amountValue = mode === 'income' ? envelope.spent : remaining;
          const progressLabel = mode === 'income'
              ...
              <Card style={styles.envelopeCard} delay={index * 35}>
                ...
                <View style={uiStyles.between}>
                  <Text style={[styles.used, { color: warning ? theme.warning : theme.muted }]}>
                    {mode === 'income' ? `${percent}% recibido` : `${percent}% usado`}
                  </Text>
                  {warning ? (
